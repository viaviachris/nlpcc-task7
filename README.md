...
本文档总结 Track1 隐式概念检索实验的模型演进、方法变化与主要指标。除特别说明外，表中指标均来自验证集检索评估结果。
...
| 对比对象 | 基线 Recall@5 | 最佳系统 Recall@5 | 绝对提升 |
|---|---:|---:|---:|
| Qwen3-0.6B Embedding LoRA 微调基线 | 0.4490 | 0.5744 | +0.1254 |
| e5-small LoRA 微调基线 | 0.3420 | 0.5744 | +0.2324 |

相较于最佳修复版 E5 单模型，最佳整体系统提升如下：

| 指标 | E5 单模型 | 最佳系统 | 绝对提升 |
|---|---:|---:|---:|
| MRR | 0.5768 | 0.7153 | +0.1385 |
| NDCG@10 | 0.4357 | 0.5973 | +0.1616 |
| Recall@10 | 0.5074 | 0.6823 | +0.1749 |
| Recall@5 | 0.4100 | 0.5744 | +0.1644 |

## 核心实验结论与消融分析

1. 底座差异：Qwen3 作为生成式架构的 embedding 模型，在复杂概念映射上显著优于传统 encoder-only 的 E5 模型。E5-small 和 E5-base 的最佳结果均停留在 `NDCG@10 = 0.43` 左右，而 Qwen3-0.6B 单模型达到 `NDCG@10 = 0.5769`，说明该任务更依赖对长文本隐式语义和受控词表概念之间关系的建模能力。

2. 互补召回：虽然 BM25 单路指标极低，`NDCG@10` 仅为 `0.1079`，但其基于字面的硬匹配与 dense embedding 的语义匹配形成互补。加入 Qwen3、E5 与 BM25 的三路 RRF 后，最佳系统达到 `NDCG@10 = 0.5973`、`Recall@5 = 0.5744`，说明字面匹配信号能补充 embedding 模型漏掉的概念词面线索。

3. 关于通用 reranker 的失败：在当前最佳 Qwen3+E5+BM25 RRF top100 候选基础上，直接使用通用 zero-shot `bge-reranker-base` 重排 top50，会将 `NDCG@10` 从 `0.5973` 大幅拉低到 `0.1092`。这说明普通 query-passage reranker 会严重破坏当前任务的排序分布，也证明在“长文本到 ELSST 受控词表概念”的检索任务中，必须引入 domain-specific 的 reranker 微调，而不能直接套用通用 reranker。
